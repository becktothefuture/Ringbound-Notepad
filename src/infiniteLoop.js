 ...
 */
export function getVirtualPositionsForPage(actualPageIndex, totalPages, scrollPosition, visibleRange) {
  const positions = [];
 ...
 */
export function getOptimalVirtualPosition(actualPageIndex, totalPages, scrollPosition, visibleRange) {
  const positions = getVirtualPositionsForPage(actualPageIndex, totalPages, scrollPosition, visibleRange);
  ...
  for (const pos of positions) {
    ...
    if (pos >= targetRange[0] && pos <= targetRange[1]) {
      ...
      distance = Math.min(
        Math.abs(pos - targetRange[0]),
        Math.abs(pos - targetRange[1])
      );
    }
